...
int unban_iteration_sleep_time = 600;
...
// In ddos info we store attack power and direction for bgp slowspec
std::map<uint32_t, banlist_item> ban_list_flowspec0;
std::map<uint32_t, banlist_item> ban_list_flowspec1;
std::map<std::string, std::string> ban_list_flowspec2;
std::map<std::string, uint32_t> ban_list_flowspec3;
std::map<uint32_t, std::vector<simple_packet> > ban_list_details_flowspec;
...
// BGP flowspec attributes by mitya
std::string exabgp_flow_spec_community = "";
std::string exabgp_flow_spec_large_community = "";
std::string exabgp_flow_spec_ext_community = "";
std::string exabgp_flow_spec_rd = "";
std::string exabgp_flow_spec_dscp = "";
std::string exabgp_flow_spec_mark = "";
std::string exabgp_action = "";
...
bool exabgp_flow_spec_ban_manage(std::string action, std::string flow_spec_rule_as_text, attack_details& current_attack);
...
bool load_configuration_file() {
    ...
    if (exabgp_enabled) {
        ...
        exabgp_flow_spec_announces = true;
        if (exabgp_flow_spec_announces) {
            exabgp_flow_spec_announces = configuration_map["exabgp_flow_spec_announces"] == "on";
            exabgp_flow_spec_community = configuration_map["exabgp_flow_spec_community"];
            if (!exabgp_flow_spec_community.empty()) {
               logger << log4cpp::Priority::INFO << "ExaBGP exabgp_flow_spec_community:" << exabgp_flow_spec_community;
            }
            exabgp_flow_spec_rd = configuration_map["exabgp_flow_spec_rd"];
            if (!exabgp_flow_spec_rd.empty()) {
               logger << log4cpp::Priority::INFO << "ExaBGP exabgp_flow_spec_rd:" << exabgp_flow_spec_rd;
            }
            exabgp_flow_spec_dscp = configuration_map["exabgp_flow_spec_dscp"];
            if (!exabgp_flow_spec_dscp.empty()) {
               logger << log4cpp::Priority::INFO << "ExaBGP exabgp_flow_spec_dscp:" << exabgp_flow_spec_dscp;
            }
            exabgp_flow_spec_community = configuration_map["exabgp_flow_spec_community"];
            if (!exabgp_flow_spec_community.empty()) {
               logger << log4cpp::Priority::INFO << "ExaBGP exabgp_flow_spec_community:" << exabgp_flow_spec_community;
            }
            exabgp_action = configuration_map["exabgp_action"];
            if (!exabgp_action.empty()) {
               if(exabgp_action.compare("redirect_vrf") == 0) {
                    logger << log4cpp::Priority::INFO << "ExaBGP exabgp_action:" << exabgp_action;
               }
            }
        }
...
bool exabgp_flow_spec_ban_manage(std::string action, std::string flow_spec_rule_as_text, attack_details& current_attack) {
    ...
    // by mitya
    int wrote_bytes = 0;
    if (action == "ban" && ban_list_flowspec3[ flow_spec_rule_as_text ] != 1) {
        ban_list_flowspec3[ flow_spec_rule_as_text ] = 1;
        wrote_bytes = write(exabgp_pipe, bgp_message.c_str(), bgp_message.size());
    ...
    if (action != "ban" && ban_list_flowspec3[ flow_spec_rule_as_text ] == 1) {
        ban_list_flowspec3[ flow_spec_rule_as_text ] = 0;
        wrote_bytes = write(exabgp_pipe, bgp_message.c_str(), bgp_message.size());
        if (wrote_bytes != bgp_message.size()) {
            logger << log4cpp::Priority::ERROR << "Can't write message to ExaBGP pipe";
            return false;
...
void cleanup_ban_list() {
    ...
    while (true) {
        ...
        boost::this_thread::sleep(boost::posix_time::seconds(global_ban_time));
        ...
        for (it = ban_list_flowspec3.begin(); it != ban_list_flowspec3.end(); it++) {
            if (it->second == 1) {
                exabgp_flow_spec_ban_manage("unban", it->first, blm);
                logger << log4cpp::Priority::INFO << "We will unban banned flow: " << it->first << "\n";
                ban_list_flowspec3[ it->first ] = 0;
                active_flow_spec_announces[ it->first ] = 0;
            }
        }
    }
}
...
void call_unban_handlers(uint32_t client_ip, attack_details& current_attack) {
    ...
    if (exabgp_enabled) {
        ...
        boost::thread exabgp_thread(exabgp_flow_spec_ban_manage, "unban", current_attack.bgp_flowspec_exa, current_attack);
        exabgp_thread.detach();
        // bool exabgp_publish_result = exabgp_flow_spec_ban_manage("ban", flow_spec_rule_text, current_attack);
    }
...
void send_attack_details(uint32_t client_ip, attack_details current_attack_details) {
    ...
    if (ban_list_details.count(client_ip) > 0) {
...
// Need to add a parameter attack_details for following write flowspec string
void produce_dpi_dump_for_pcap_dump(std::string pcap_file_path, std::stringstream& ss, std::string client_ip_as_string, attack_details& current_attack) {
    ...
    while (1) {
        ...
        logger << log4cpp::Priority::INFO << "PROTO from DPI:" << detected_protocol.protocol << "\n";
    ...
    } else if ( current_attack.attack_protocol == 1) {
        attack_type = AMPLIFICATION_ATTACK_ICMP;
        logger << log4cpp::Priority::INFO << "Attack protocol: " << current_attack.attack_protocol << "\n";
    }
...
//            if (itr == active_flow_spec_announces.end()) {
                ...
                current_attack.bgp_flowspec_exa = flow_spec_rule_text;
...
void call_attack_details_handlers(uint32_t client_ip, attack_details& current_attack, std::string attack_fingerprint) { 
    ...
    if (collect_attack_pcap_dumps && process_pcap_attack_dumps_with_dpi) {
        ...
        // Need to add argument attack_details
        produce_dpi_dump_for_pcap_dump(attack_pcap_dump_path, string_buffer_for_dpi_data, client_ip_as_string, current_attack);
...
std::string generate_flow_spec_for_amplification_attack(amplification_attack_type_t amplification_attack_type, std::string destination_ip) {
    ...
    if(exabgp_action.compare("redirect_vrf") == 0) {
        logger << log4cpp::Priority::INFO << "ExaBGP exabgp_action:" << exabgp_action;
        my_action.set_type(FLOW_SPEC_ACTION_REDIRECT);
        if (!exabgp_flow_spec_rd.empty()) {
            logger << log4cpp::Priority::INFO << "ExaBGP exabgp_flow_spec_rd:" << exabgp_flow_spec_rd;
            my_action.set_rd(exabgp_flow_spec_rd); 
            if (!exabgp_flow_spec_dscp.empty()) {
                exabgp_rule.set_dscp(exabgp_flow_spec_dscp);
            }
            if (!exabgp_flow_spec_community.empty()) {
                my_action.set_community(exabgp_flow_spec_community);
            }
        }
    }
    ...
    if (amplification_attack_type == AMPLIFICATION_ATTACK_DNS) {
        ...
        exabgp_rule.add_protocol(FLOW_SPEC_PROTOCOL_UDP);
    } else if (amplification_attack_type == AMPLIFICATION_ATTACK_ICMP) {
